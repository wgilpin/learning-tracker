...
async def _create_chapter(async_session: AsyncSession, user_id: uuid.UUID) -> tuple[uuid.UUID, uuid.UUID]:
    ...
    topic = await create_topic(async_session, TopicCreate(title="Comment Test Topic"), user_id=user_id)
...
async def test_post_comment_returns_200(
    ...
    chapter_id, _ = await _create_chapter(async_session, test_user.id)
